...
class NeighborSamplerOrigin(NeighborSampler):
    ...
    def __init__(self, adj_list: list, in_adj_list: list, out_adj_list: list, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
        """
        Neighbor sampler.
...
def get_neighbor_sampler(data: Data, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None, fix_window_size: int = -1,device: str='cpu', undirected=True):
    ...
    in_adj_list = [[] for _ in range(max_node_id + 1)]
    out_adj_list = [[] for _ in range(max_node_id + 1)]
    for src_node_id, dst_node_id, edge_id, node_interact_time in zip(data.src_node_ids, data.dst_node_ids, data.edge_ids, data.node_interact_times):
        ...
        in_adj_list[dst_node_id].append((src_node_id, edge_id, node_interact_time))
        out_adj_list[src_node_id].append((dst_node_id, edge_id, node_interact_time))

    return NeighborSamplerOrigin(adj_list=adj_list, in_adj_list=in_adj_list, out_adj_list=out_adj_list, sample_neighbor_strategy=sample_neighbor_strategy, time_scaling_factor=time_scaling_factor, seed=seed)
