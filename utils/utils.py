...
from tgb_seq.datasets.datasets_info import DATA_VERSION

TIME_ENCODING_DICT = { # time_base, max_slot_length
    'wikipedia': [10, 300000],
    'mooc': [10, 300000],
    'reddit': [10, 300000],
    'lastfm': [500, 300000],
    ...
    'Contacts': [300, 9000],
    'Flights': [150, 1500],
    'SocialEvo': [210, 100000],
    'uci': [20, 1000000],
    'UNtrade': [1000000, 1000],
    'UNvote': [2300000, 1000],
    'USLegis': [1, 100],
    'YouTube': [10000, 2000],
    # not prepared
    'tgbl-review': [10, 300000],
    'tgbl-wiki': [10, 300000],
    'tgbl-coin': [10, 300000],  
    'tgbl-flight': [10, 300000],
    'tgbl-comment': [10, 300000],
    'Flickr': [10, 300000],
    'GoogleLocal': [10, 300000],
    'WikiLink': [10, 300000],
    'Patent': [10, 300000],
    'Yelp': [10, 300000],
    'ML-20M': [10, 300000],
...
TIME_SLOT_DICT = { # time_base,slot_len
    'wikipedia': [1000, 3000],
    'mooc': [10, 300000],
    'reddit': [10, 300000],
    'lastfm': [500, 300000],
...
def create_optimizer(model: nn.Module, optimizer_name: str, learning_rate: float, weight_decay: float = 0.0, betas= None):
    """
    create optimizer
    :param model: nn.Module
    ...
    if optimizer_name == 'Adam':
        if betas is not None:
            optimizer = torch.optim.Adam(params=model.parameters(), lr=learning_rate, weight_decay=weight_decay, betas=betas)
        else:
...
def write_results(save_model_folder, model_name, version, dataset_path, dataset, mrr_list, pos_rank_list, pos_scores, first_list, y_pred_first_list, last_nei_time_list):
    if dataset in DATA_VERSION:
        dataset_v = DATA_VERSION[dataset]
    else:
        dataset_v = '0'
    if dataset_v == '0':
        df=pd.read_csv(f'{dataset_path}/{dataset}/{dataset}.csv')
    else:
        df=pd.read_csv(f'{dataset_path}/{dataset}/{dataset}_v{dataset_v}.csv')
    df['mrr']=0
    df['pos_prefix']='*'
    df['pos_node']=-1
    df['pos_rank']=-1
    ...
    for i in range(20):
        df[f'first_{i+1}']=-1
        df[f'first_{i+1}_score']=-1
        df[f'first_{i+1}_time']=-1
        df[f'first_{i+1}_num_nei']=-1
    if 'split' in df.columns:
        test_df = df[df['split'] == 2]
    else:
        test_df = df[df['ext_roll']==2]
    last_nei_time_list, neislen_list = last_nei_time_list[0], last_nei_time_list[1]
    assert len(test_df) == len(mrr_list) == len(pos_rank_list) == len(first_list) == len(y_pred_first_list) == len(last_nei_time_list) == len(neislen_list)
    test_df['mrr'] = mrr_list
    test_df['pos_rank'] = np.array(pos_rank_list)[:,1]
    test_df['pos_node'] = np.array(pos_rank_list)[:,0]
    test_df['pos_score'] = torch.tensor(pos_scores).cpu().numpy()
    test_df['src_last_time'] = np.array(last_nei_time_list)[:,0]
    test_df['dst_last_time'] = np.array(last_nei_time_list)[:,1]
    for i in range(20):
        test_df[f'first_{i+1}'] = np.array(first_list)[:,i]
        test_df[f'first_{i+1}_score'] = np.array(y_pred_first_list)[:,i]
        test_df[f'first_{i+1}_time'] = np.array(last_nei_time_list)[:,i+2]
        test_df[f'first_{i+1}_num_nei'] = np.array(neislen_list)[:,i+2]
    new_df = pd.concat([df[df['split'] != 2], test_df], axis=0)
    new_df.to_csv(f'{save_model_folder}/{dataset}_{model_name}_v{version}_test_results.csv', index=False)
...
class NeighborSamplerOrigin(NeighborSampler):
    ...
    def find_neighbors_at_time(self, node_id: int, interact_time: float):
        """
        extracts all the interactions happening at interact_time for node_id in the overall interaction graph
        ...
        j = np.searchsorted(self.nodes_neighbor_times[node_id], interact_time, side='right')

        return self.nodes_neighbor_ids[node_id][i:j]
    ...
    def find_interact_time_before(self, node_id: np.ndarray, interact_time: np.ndarray):
        """
        extracts all the interactions happening before interact_time (less than interact_time) for node_id in the overall interaction graph
        the returned interactions are sorted by time.
        :param node_id: int, node id
        ...
        # return index i, which satisfies list[i - 1] < v <= list[i]
        # return 0 for the first position in self.nodes_neighbor_times since the value at the first position is empty
        last_interact_times = np.zeros_like(node_id)
        for k, (id, t) in enumerate(zip(node_id, interact_time)):
            i = np.searchsorted(self.nodes_neighbor_times[id], t)
            if i >= 1:
                last_interact_times[k] = self.nodes_neighbor_times[id][i-1]
            else:
                last_interact_times[k] = 0
        return last_interact_times
...
class LastUpdateTime(object):
    def __init__(self, num_nodes: int, chrono: bool = True, neighbor_sampler: NeighborSampler = None):
        self.last_update_time = np.zeros(num_nodes+1, dtype=np.float32)
        self.train_val_last_update_time = None
        self.chrono = chrono
        self.neighbor_sampler = neighbor_sampler
    
    def update(self, src_node_id: np.ndarray, dst_node_id: np.ndarray, update_time: np.ndarray):
        if self.chrono:
            self.last_update_time[src_node_id] = update_time
            self.last_update_time[dst_node_id] = update_time
        

    def get_last_update_time(self, node_id: np.ndarray, node_interact_times: np.ndarray = None):
        if self.chrono:
            return self.last_update_time[node_id]
        else:
            return self.neighbor_sampler.find_interact_time_before(node_id=node_id, interact_time=node_interact_times)

    def reset(self):
        self.last_update_time = np.zeros(len(self.last_update_time), dtype=np.float32)
    
    def backup_state(self):
        self.train_val_last_update_time = self.last_update_time.copy()

    def recover_state(self):
        self.last_update_time = self.train_val_last_update_time.copy()

def get_neighbor_sampler(data: Data, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None, fix_window_size: int = -1,device: str='cpu', undirected=True):
    """
    ...
    :return:
    """
    if fix_window_size!=-1 and sample_neighbor_strategy=='recent':
        num_nodes=data.src_node_ids.max()+1 if data.src_node_ids.max()>data.dst_node_ids.max() else data.dst_node_ids.max()+1
        return NeighborSamplerWindow(num_nodes=num_nodes,fix_window_size=fix_window_size,device=device,undirected=undirected)
...
class NeighborSamplerWindow(NeighborSampler):
    def __init__(self,num_nodes,fix_window_size=20, device: str = 'cpu',undirected=True, time_scaling_factor=0.1):
        ...
        self.neighbors_arr=torch.zeros((num_nodes,fix_window_size)).long()
        self.edgeids_arr=torch.zeros((num_nodes,fix_window_size)).long()
        self.times_arr=torch.zeros((num_nodes,fix_window_size))
        self.num_neighbors_arr=torch.zeros((num_nodes)).long()
    ...
    def reset(self):
        self.neighbors_arr=torch.zeros((self.num_nodes,self.fix_window_size)).long()
        self.edgeids_arr=torch.zeros((self.num_nodes,self.fix_window_size)).long()
        self.times_arr=torch.zeros((self.num_nodes,self.fix_window_size))
    
    def backup_state(self):
        self.train_val_neighbors_arr=self.neighbors_arr.detach().copy()
        self.train_val_edgeids_arr=self.edgeids_arr.detach().copy()
        self.train_val_times_arr=self.times_arr.detach().copy()
    
    def recover_state(self):
        self.neighbors_arr=self.train_val_neighbors_arr.detach().copy()
        self.edgeids_arr=self.train_val_edgeids_arr.detach().copy()
        self.times_arr=self.train_val_times_arr.detach().copy()
...
class NegativeEdgeSampler(object):
    ...
    def sample_with_time_collision_check(self, num_negs: int, batch_src_node_ids: np.ndarray, batch_node_interact_times: np.ndarray, neighbor_sampler: NeighborSampler):
        neg_dst_node_ids = []
        for s,t in zip(batch_src_node_ids, batch_node_interact_times):
            neighbors = neighbor_sampler.find_neighbors_at_time(s,t)
            candidate_dsts = np.setdiff1d(self.unique_dst_node_ids, neighbors)
            rdm_indices = self.random_state.choice(len(candidate_dsts), size=num_negs, replace=len(candidate_dsts) < num_negs)
            neg_dst_node_ids.append(candidate_dsts[rdm_indices])
        return np.array(neg_dst_node_ids)


def pad_sequences(nodes_neighbor_ids_list: list, nodes_edge_ids_list: list = None,
                    nodes_neighbor_times_list: list = None, max_seq_length: int = 256):
    """
    ...
    :return:
    """
    if nodes_edge_ids_list is None:
        padded_nodes_neighbor_ids = np.zeros(
        (len(nodes_neighbor_ids_list), max_seq_length)).astype(np.longlong)
        for idx in range(len(nodes_neighbor_ids_list)):
            if len(nodes_neighbor_ids_list[idx]) > 0:
                padded_nodes_neighbor_ids[idx, 0: len(
                    nodes_neighbor_ids_list[idx])] = nodes_neighbor_ids_list[idx]
        return padded_nodes_neighbor_ids

    # pad the sequences
    # three ndarrays with shape (batch_size, max_seq_length)
    padded_nodes_neighbor_ids = np.zeros(
        (len(nodes_edge_ids_list), max_seq_length)).astype(np.longlong)
    padded_nodes_edge_ids = np.zeros(
        (len(nodes_edge_ids_list), max_seq_length)).astype(np.longlong)
    padded_nodes_neighbor_times = np.zeros(
        (len(nodes_edge_ids_list), max_seq_length)).astype(np.float32)

    for idx in range(len(nodes_edge_ids_list)):
        if len(nodes_neighbor_ids_list[idx]) > 0:
            padded_nodes_neighbor_ids[idx, 0: len(
                nodes_neighbor_ids_list[idx])] = nodes_neighbor_ids_list[idx]
            padded_nodes_edge_ids[idx, 0: len(
                nodes_edge_ids_list[idx])] = nodes_edge_ids_list[idx]
            padded_nodes_neighbor_times[idx, 0: len(
