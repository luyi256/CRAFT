...
class NeighborCooccurrenceEncoder(nn.Module):
    ...
    def __init__(self, neighbor_co_occurrence_feat_dim: int, device: str = 'cpu', only_repeat_time: bool = False):
        """
        ...
        self.device = device
        self.only_repeat_time = only_repeat_time
    ...
    def forward(self, src_padded_nodes_neighbor_ids: np.ndarray, dst_padded_nodes_neighbor_ids: np.ndarray):
        ...
        src_padded_nodes_appearances, dst_padded_nodes_appearances = self.count_nodes_appearances(src_padded_nodes_neighbor_ids=src_padded_nodes_neighbor_ids,dst_padded_nodes_neighbor_ids=dst_padded_nodes_neighbor_ids)
        ...
        # src_padded_nodes_neighbor_co_occurrence_features, Tensor, shape (batch_size, src_max_seq_length, neighbor_co_occurrence_feat_dim)
        # dst_padded_nodes_neighbor_co_occurrence_features, Tensor, shape (batch_size, dst_max_seq_length, neighbor_co_occurrence_feat_dim)
        if self.only_repeat_time:
            src_padded_nodes_neighbor_co_occurrence_features[:, 1:, :] = 0.0
            dst_padded_nodes_neighbor_co_occurrence_features[:, 1:, :] = 0.0
        return src_padded_nodes_neighbor_co_occurrence_features, dst_padded_nodes_neighbor_co_occurrence_features


class TransformerEncoder(nn.Module):

    def __init__(self, attention_dim: int, num_heads: int, dropout: float = 0.1, separate_norm: bool = False):
        """
        Transformer encoder.
        ...
        self.dim_per_head = attention_dim//num_heads
        if separate_norm:
            norm_layers_list = [
                nn.LayerNorm(self.dim_per_head)
                for _ in range(num_heads)
            ]
            self.norm_layers = nn.ModuleList(norm_layers_list*2)
        else:
    ...
    def forward(self, inputs: torch.Tensor, attn_mask: torch.Tensor = None, separate_norm: bool = False):
        """
        ...
        if self.separate_norm:
            normalized_inputs = transposed_inputs.clone()
            for i in range(self.num_heads):
                tmp = transposed_inputs[:,:,i*self.dim_per_head:(i+1)*self.dim_per_head]
                tmp = self.norm_layers[i](tmp)
                normalized_inputs[:,:,i*self.dim_per_head:(i+1)*self.dim_per_head] = tmp
            transposed_inputs = normalized_inputs
        else:
            transposed_inputs = self.norm_layers[0](transposed_inputs)
        # Tensor, shape (batch_size, num_patches, self.attention_dim)
        hidden_states = self.multi_head_attention(query=transposed_inputs, key=transposed_inputs, value=transposed_inputs, attn_mask=attn_mask)[0].transpose(0, 1)
        # Tensor, shape (batch_size, num_patches, self.attention_dim)
        outputs = inputs + self.dropout(hidden_states)
        # Tensor, shape (batch_size, num_patches, self.attention_dim)
        if self.separate_norm:
            normalized_states = outputs.clone()
            for i in range(self.num_heads):
                tmp = outputs[:,:,i*self.dim_per_head:(i+1)*self.dim_per_head]
                tmp = self.norm_layers[i+self.num_heads](tmp)
                normalized_states[:,:,i*self.dim_per_head:(i+1)*self.dim_per_head] = tmp
            outputs = normalized_states
        else:
            outputs = self.norm_layers[1](outputs)
        hidden_states = self.linear_layers[1](self.dropout(F.gelu(self.linear_layers[0](outputs))))
        # Tensor, shape (batch_size, num_patches, self.attention_dim)
