...
class MemoryModel(torch.nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        # memory modules
        self.memory_bank = MemoryBank(num_nodes=self.num_nodes, memory_dim=self.memory_dim, message_dim=self.message_dim)
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, neg_dst_node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        node_ids = np.concatenate([src_node_ids, dst_node_ids, neg_dst_node_ids])
        ...
        # t_update_mem_start=time.perf_counter()

        reversed_idx = np.arange(len(node_ids) - 1, -1, -1)
        unique_node_ids, last_indices_reversed = np.unique(node_ids[::-1], return_index=True)
        last_indices = reversed_idx[last_indices_reversed]
        unique_node_interact_times = node_interact_times[last_indices]
        neighbors=self.neighbor_sampler.get_historical_neighbors(node_ids=unique_node_ids,node_interact_times=unique_node_interact_times,num_neighbors=num_neighbors)
        nodes_needed_updated=np.concatenate([unique_node_ids, neighbors[0].flatten()])
        updated_node_memories, updated_node_last_updated_times = self.get_updated_memories(node_ids=nodes_needed_updated,node_raw_messages=self.memory_bank.node_raw_messages)
        # t_update_mem_end=time.perf_counter()
        # compute the node temporal embeddings using the embedding module
        src_node_interaction_times = node_interact_times[:len(src_node_ids)]
        dst_node_interaction_times = node_interact_times[len(src_node_ids): len(src_node_ids) + len(dst_node_ids)]
        neg_dst_node_interaction_times = node_interact_times[len(src_node_ids) + len(dst_node_ids): ]
        if self.model_name == 'JODIE':
            ...
            src_node_time_intervals = torch.from_numpy(src_node_interaction_times).float().to(self.device) - updated_node_last_updated_times[torch.from_numpy(src_node_ids)]
            src_node_time_intervals = (src_node_time_intervals - self.src_node_mean_time_shift) / self.src_node_std_time_shift
            # Tensor, shape (batch_size, )
            dst_node_time_intervals = torch.from_numpy(dst_node_interaction_times).float().to(self.device) - updated_node_last_updated_times[torch.from_numpy(dst_node_ids)]
            dst_node_time_intervals = (dst_node_time_intervals - self.dst_node_mean_time_shift_dst) / self.dst_node_std_time_shift
            # Tensor, shape (2 * batch_size, )
            neg_dst_node_time_intervals = torch.from_numpy(neg_dst_node_interaction_times).float().to(self.device) - updated_node_last_updated_times[torch.from_numpy(neg_dst_node_ids)]
            neg_dst_node_time_intervals = (neg_dst_node_time_intervals - self.dst_node_mean_time_shift_dst) / self.dst_node_std_time_shift
            node_time_intervals = torch.cat([src_node_time_intervals, dst_node_time_intervals, neg_dst_node_time_intervals], dim=0)
            ...
            node_embeddings = self.embedding_module.compute_node_temporal_embeddings(node_memories=updated_node_memories,
                                                                                     node_ids=node_ids,
                                                                                     node_interact_times=node_interact_times,current_layer_num=self.num_layers,num_neighbors=num_neighbors)
        else:
            raise ValueError(f'Not implemented error for model_name {self.model_name}!')
        
        # two Tensors, with shape (batch_size, node_feat_dim)
        src_node_embeddings, dst_node_embeddings, neg_dst_node_embeddings = node_embeddings[:len(src_node_ids)], node_embeddings[len(src_node_ids): len(src_node_ids) + len(dst_node_ids)], node_embeddings[len(src_node_ids) + len(dst_node_ids): ]
        # t_conv_end=time.perf_counter()
        
        assert edge_ids is not None
        pos_node_ids=np.concatenate([src_node_ids,dst_node_ids])
        # if the edges are positive, update the memories for source and destination nodes (since now we have new messages for them)
        self.update_memories(node_ids=pos_node_ids, node_raw_messages=self.memory_bank.node_raw_messages)

        # clear raw messages for source and destination nodes since we have already updated the memory using them
        self.memory_bank.clear_node_raw_messages(node_ids=pos_node_ids)

        # compute new raw messages for source and destination nodes
        self.compute_new_node_raw_messages(src_node_ids=src_node_ids,
                                                                                            dst_node_ids=dst_node_ids,
                                                                                            dst_node_embeddings=dst_node_embeddings,
                                                                                            node_interact_times=src_node_interaction_times,
                                                                                            edge_ids=edge_ids, node_raw_messages=self.memory_bank.node_raw_messages)
        self.compute_new_node_raw_messages(src_node_ids=dst_node_ids,
                                                                                            dst_node_ids=src_node_ids,
                                                                                            dst_node_embeddings=src_node_embeddings,
                                                                                            node_interact_times=dst_node_interaction_times,
                                                                                            edge_ids=edge_ids, node_raw_messages=self.memory_bank.node_raw_messages)
        ...
        if self.model_name == 'DyRep':
            ...
            neg_dst_node_embeddings = updated_node_memories[torch.from_numpy(neg_dst_node_ids)]
        # t_update_msg_end=time.perf_counter()
        # t_update_mem=t_update_mem_end-t_update_mem_start
        # t_conv=t_conv_end-t_update_mem_end
        # t_update_msg=t_update_msg_end-t_conv_end
        return src_node_embeddings, dst_node_embeddings, neg_dst_node_embeddings
    ...
    def get_updated_memories(self, node_ids: np.ndarray, node_raw_messages: dict):
        ...
        # t_aggr_start=time.perf_counter()
        unique_node_ids, unique_node_messages, unique_node_timestamps = self.message_aggregator.aggregate_messages(node_ids=node_ids,node_raw_messages=node_raw_messages)
        # get updated memory for all nodes with messages stored in previous batches (just for computation)
        # t_aggr_end=time.perf_counter()
        # updated_node_memories, Tensor, shape (num_nodes, memory_dim)
        # updated_node_last_updated_times, Tensor, shape (num_nodes, )
        updated_node_memories, updated_node_last_updated_times = self.memory_updater.get_updated_memories(unique_node_ids=unique_node_ids,unique_node_messages=unique_node_messages,unique_node_timestamps=unique_node_timestamps)
        # t_update_end=time.perf_counter()
        # return updated_node_memories, updated_node_last_updated_times, len(unique_node_ids), t_update_end-t_aggr_end, t_aggr_end-t_aggr_start
        return updated_node_memories, updated_node_last_updated_times

    def update_memories(self, node_ids: np.ndarray, node_raw_messages: nn.Parameter):
        """
    ...
    def compute_new_node_raw_messages(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, dst_node_embeddings: torch.Tensor,
                                      node_interact_times: np.ndarray, edge_ids: np.ndarray, node_raw_messages: np.ndarray):
        """
        ...
        new_node_raw_messages=torch.cat([new_src_node_raw_messages,torch.from_numpy(node_interact_times).unsqueeze(1).to(new_src_node_raw_messages.device)],dim=1)
        for i in range(len(src_node_ids)):
            node_raw_messages.data[src_node_ids[i]]=new_node_raw_messages[i]
        # for i in range(len(src_node_ids)):
...
class MessageAggregator(nn.Module):
    ...
    def aggregate_messages(self, node_ids: np.ndarray, node_raw_messages: np.ndarray):
        """
        ...
        unique_node_messages, unique_node_timestamps, to_update_node_ids = node_raw_messages.data[unique_node_ids,:-1], node_raw_messages.data[unique_node_ids,-1], unique_node_ids
...
class MemoryBank(nn.Module):
    ...
    def __init__(self, num_nodes: int, memory_dim: int, message_dim: int):
        ...
        self.node_raw_messages = nn.Parameter(torch.zeros((self.num_nodes, message_dim+1)),requires_grad=False)
    ...
    def __init_memory_bank__(self):
        ...
        self.node_raw_messages.data.zero_()
    ...
    def backup_memory_bank(self):
        ...
        return self.node_memories.data.clone(), self.node_last_updated_times.data.clone(), self.node_raw_messages.data.clone()
    ...
    def reload_memory_bank(self, backup_memory_bank: tuple):
        ...
        :return:
        """
        self.node_memories.data, self.node_last_updated_times.data, self.node_raw_messages.data = backup_memory_bank[0].clone(), backup_memory_bank[1].clone(), backup_memory_bank[2].clone()
    ...
    def detach_memory_bank(self):
        ...
        self.node_raw_messages.detach_()
    ...
    def store_node_raw_messages(self, node_ids: np.ndarray, new_node_raw_messages: np.ndarray):
        """
        ...
        :return:
        """
        self.node_raw_messages.data[node_ids]=new_node_raw_messages
    ...
    def clear_node_raw_messages(self, node_ids: np.ndarray):
        ...
        if node_ids is None:
            return
        self.node_raw_messages.data[node_ids,:]=0

    def get_node_last_updated_times(self, unique_node_ids: torch.Tensor):
        """
...
class MemoryUpdater(nn.Module):
    ...
    def update_memories(self, unique_node_ids: np.ndarray, unique_node_messages: torch.Tensor,
        ...
        unique_node_ids = torch.from_numpy(unique_node_ids).to(self.memory_bank.node_memories.device)
        assert (self.memory_bank.get_node_last_updated_times(unique_node_ids) <=
                unique_node_timestamps).all().item(), "Trying to update memory to time in the past!"
    
        # only the last timestamp which is **less** than the current timestamp will be updated
        # the other timestamps will be ignored
        to_update_node_ids = unique_node_ids[unique_node_timestamps > self.memory_bank.get_node_last_updated_times(unique_node_ids)]
        to_update_node_messages = unique_node_messages[unique_node_timestamps > self.memory_bank.get_node_last_updated_times(unique_node_ids)]
        to_update_node_timestamps = unique_node_timestamps[unique_node_timestamps > self.memory_bank.get_node_last_updated_times(unique_node_ids)]
        
        # Tensor, shape (num_unique_node_ids, memory_dim)
        node_memories = self.memory_bank.get_memories(node_ids=to_update_node_ids)
        # Tensor, shape (num_unique_node_ids, memory_dim)
        updated_node_memories = self.memory_updater(to_update_node_messages, node_memories)
        # update memories for nodes in unique_node_ids
        self.memory_bank.set_memories(node_ids=to_update_node_ids, updated_node_memories=updated_node_memories)

        # update last updated times for nodes in unique_node_ids
        self.memory_bank.node_last_updated_times[to_update_node_ids] = to_update_node_timestamps
    ...
    def get_updated_memories(self, unique_node_ids: np.ndarray, unique_node_messages: torch.Tensor,
        ...
        updated_node_memories[to_update_node_ids] = self.memory_updater(to_update_node_messages,
                                                                                       updated_node_memories[to_update_node_ids])

        # Tensor, shape (num_nodes, )
        updated_node_last_updated_times = self.memory_bank.node_last_updated_times.data.clone()
        updated_node_last_updated_times[to_update_node_ids] = to_update_node_timestamps
